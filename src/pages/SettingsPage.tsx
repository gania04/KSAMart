              ...
              <div className="p-6 space-y-4">
                ...
                {qrisEnabled && (
                  <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                    ...
                    {isUploadingQris && (
                      <div className="flex items-center gap-2 bg-fuchsia-50 border border-fuchsia-200 rounded-lg p-3 text-xs text-fuchsia-800">
                        <svg className="animate-spin h-4 w-4 text-fuchsia-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        <span className="font-semibold">Mengupload QRIS ke cloud... Mohon tunggu sebentar.</span>
                      </div>
                    )}
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (file) {
                          ...
                          try {
                            ...
                            const { file: compressedFile, base64 } = await compressImage(file, 600, 0.8);
                            
                            // Upload ke Supabase Storage bucket 'store-assets'
                            const path = `qris/qris_ksamart_${Date.now()}.jpg`;
                            const url = await uploadImageToStorage(compressedFile, 'store-assets', path, base64);
                            setQrisImageUrl(url);
                          } catch (err) {
                      ...
                      className={`w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-fuchsia-50 file:text-fuchsia-700 hover:file:bg-fuchsia-100 cursor-pointer border border-gray-200 p-1 rounded-lg ${isUploadingQris ? 'opacity-50 cursor-not-allowed' : ''}`}
                    />
                    {qrisImageUrl && !isUploadingQris && (
                      <div className="mt-2 relative w-36 h-36 border-2 border-fuchsia-200 rounded-xl overflow-hidden shadow-sm">
                        <img src={qrisImageUrl} alt="QRIS" className="w-full h-full object-contain bg-white p-1" />
                        {qrisImageUrl.startsWith('http') && (
                          <div className="absolute bottom-0 left-0 right-0 bg-fuchsia-600/90 text-white text-[8px] font-bold text-center py-1">
                            ✅ Cloud Storage — Terlihat di semua perangkat
                          </div>
                        )}
                        <button
                          type="button"
                          onClick={() => setQrisImageUrl('')}
                          className="absolute top-1 right-1 bg-white/90 p-1 rounded-lg text-red-500 hover:bg-red-50 border border-red-100 shadow-xs"
                          title="Hapus QRIS"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M19 6l-1 14H6L5 6M10 11v6M14 11v6M9 6V4h6v2"/></svg>
                        </button>
                      </div>
                    )}
                    <p className="text-[10px] text-gray-400 mt-1">Upload barcode QRIS toko Anda agar pelanggan bisa menscan langsung saat checkout. Gambar tersimpan di cloud dan bisa dilihat dari semua perangkat.</p>
                  </div>
                )}
